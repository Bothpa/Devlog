...
//     const [Categorys, setCategorys] = useState<string[]>(["카테고리를 불러오는 중입니다."]);
...
//         .then((res)=>{
//           if(res.status === 200){
//             const categoryNames = res.data.map((category: any) => category.cateName);
//             setCategorys(categoryNames);
//             console.log(res.data);
//           }
//         })
//         .catch((err) => {
//           alert("카테고리 불러오기중 오류가 발생하였습니다.");
//         });
//       }, []);
...
const SettingCategory = () => {
    ...
    const AddCategory = useCallback(() => {
        if (Categorys.length >= 10) {
            ...
            // const newCategory = {
            //     cateName: `새로운 카테고리 ${Categorys.length + 1}`,
            //     cateUuid: Date.now(),
            //     cateIdx: Categorys.length + 1,
            // };
            // setCategorys([...Categorys, newCategory]);
            TokenAxios.post("/cate", {cateName: `새 카테고리${Categorys.length + 1}`, cateIdx: Categorys.length + 1})
            .then((res) => {
                if (res.status === 200) {
                    const categoryData = {
    ...
    return (
        <div className="w-full h-full flex flex-col">
            ...
            <div className="flex flex-col w-full h-fit px-8 py-6 border mb-10 bg-white">
                ...
                <div className="flex flex-col w-full h-fit px-8 py-6 border mb-10 bg-[#E7EDF3]">
                <div className="flex h-[61px] flex-row items-center bg-white pr-3 text-xl border-b pl-6">전체보기</div>
                    <Reorder.Group axis="y" onReorder={setCategorys} values={Categorys} className="ml-0">
                        {Categorys.map((item) => (
