...
const SettingTCategory = () => {
    ...
    const AddCategory = useCallback(() => {
        if (Categorys.length >= 10) {
            ...
            TokenAxios.post("/cate/tBlog", {cateName: `새 카테고리${hour+minute+second}`, cateIdx: Categorys.length == 0 ? 0 : Categorys[Categorys.length -1].cateIdx + 1})
            .then((res) => {
                if (res.status === 200) {
                    const categoryData = {
    ...
    const SaveCateChanges = useCallback(() => {
        ...
        TokenAxios.patch("/cate/tBlog", categoryData)
        .then((res) => {
            if (res.status === 200) {
...
export default SettingTCategory;
